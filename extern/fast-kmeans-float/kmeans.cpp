...
void Kmeans::verifyAssignment(int iteration, int startNdx, int endNdx) const {
    ...
    for (int i = startNdx; i < endNdx; ++i) {
        ...
        float closest_dist2 = pointCenterDist2(i, closest);
        float original_closest_dist2 = closest_dist2;
        // look at all centers
        for (int j = 0; j < k; ++j) {
            if (j == closest) {
                continue;
            }
            float d2 = pointCenterDist2(i, j);
