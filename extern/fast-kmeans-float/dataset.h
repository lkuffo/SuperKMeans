...
class Dataset {
    public:
        ...
        Dataset(int aN, int aD, bool keepSDS = false) : n(aN), d(aD), nd(n * d), 
                                      data(new float[nd]),
                                      sumDataSquared(keepSDS ?  new float[n] : NULL) {}
        ...
        ~Dataset() {
            n = d = nd = 0; 
            float *dp = data, *sdsp = sumDataSquared;
        ...
        float &operator()(int ndx, int dim);

        // allows const access to record ndx and dimension dim
        const float &operator()(int ndx, int dim) const;
