...
void addVectors(float *a, float const *b, int d) {
    float const *end = a + d;
    while (a < end) {
        *(a++) += *(b++);
    }
...
void subVectors(float *a, float const *b, int d) {
    float const *end = a + d;
    while (a < end) {
        *(a++) -= *(b++);
    }
...
float distance2silent(float const *a, float const *b, int d) {
    float d2 = 0.0, diff;
    float const *end = a + d; // one past the last valid entry in a
    while (a < end) {
        diff = *(a++) - *(b++);
        d2 += diff * diff;
    }
...
void centerDataset(Dataset *x) {
    ...
    const float *xEnd = x->data + x->n * x->d;
    for (float *xp = x->data; xp != xEnd; xp += x->d) {
...
Dataset *init_centers(Dataset const &x, unsigned short k) {
    ...
    for (int i = 0; i < k; ++i) {
        ...
        float *cdp = c->data + i * x.d;
        memcpy(cdp, x.data + chosen_pts[i] * x.d, sizeof(float) * x.d);
        if (c->sumDataSquared) {
...
Dataset *init_centers_kmeanspp(Dataset const &x, unsigned short k) {
    ...
    float *distribution = new float[x.n];
    ...
    while (ndx < k) {
        ...
        float *new_center_ptr = std::lower_bound(distribution, distribution + x.n, r);
...
float getMemoryUsage() {
    ...
    float sizeInKilobytes = residentSetSizeInPages * 4.0; // assumes 4096 byte page
    // getconf PAGESIZE
...
void assign(Dataset const &x, Dataset const &c, unsigned short *assignment) {
    for (int i = 0; i < x.n; ++i) {
        float shortestDist2 = std::numeric_limits<float>::max();
        int closest = 0;
        for (int j = 0; j < c.n; ++j) {
            float d2 = 0.0, *a = x.data + i * x.d, *b = c.data + j * x.d;
...
float elapsed_time(rusage *start) {
    rusage now;
    timeval diff;
    getrusage(RUSAGE_SELF, &now);
