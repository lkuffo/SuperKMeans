...
// "ndx"
float &Dataset::operator()(int ndx, int dim) {
#   ifdef DEBUG
    assert(ndx < n); 
    assert(dim < d); 
#   endif
...
const float &Dataset::operator()(int ndx, int dim) const {
#   ifdef DEBUG
    assert(ndx < n); 
    assert(dim < d); 
#   endif
...
Dataset const &Dataset::operator=(Dataset const &x) {
    if (this != &x) {
        ...
        if (n != x.n) {
            ...
            sumDataSquared = x.sumDataSquared ? new float[x.n] : NULL;
        }

        if (nd != x.nd) {
            delete [] data;
            data = x.data ? new float[x.nd] : NULL;
        }
        ...
        if (x.sumDataSquared) {
            memcpy(sumDataSquared, x.sumDataSquared, x.n * sizeof(float));
        }

        if (x.data) {
            memcpy(data, x.data, x.nd * sizeof(float));
        }
