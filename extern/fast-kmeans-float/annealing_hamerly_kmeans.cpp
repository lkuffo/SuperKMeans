...
int AnnealingHamerlyKmeans::runThread(int threadId, int maxIterations) {
    ...
    float changedLastIter = 0.0;
    float maxChanged = 0.0;
    ...
    while ((iterations < maxIterations) && ! converged) {
        ...
        for (int i = startNdx; i < endNdx; ++i) {
            ...
            float l2 = std::numeric_limits<float>::max(); // the squared lower bound
            for (int j = 0; j < k; ++j) {
                if (j == closest) { continue; }


                float dist2 = pointCenterDist2(i, j);
...
void AnnealingHamerlyKmeans::update_bounds(int startNdx, int endNdx) {
    float longest = centerMovement[0], secondLongest = (1 < k) ? centerMovement[1] : centerMovement[0];
