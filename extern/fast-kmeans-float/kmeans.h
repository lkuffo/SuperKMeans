...
class Kmeans {
    public:
        ...
        // functions.
        virtual float pointPointInnerProduct(int x1, int x2) const = 0;
        virtual float pointCenterInnerProduct(int xndx, unsigned short cndx) const = 0;
        virtual float centerCenterInnerProduct(unsigned short c1, unsigned short c2) const = 0;

        // Use the inner products to compute squared distances between a point
        // and center.
        virtual float pointCenterDist2(int x1, unsigned short cndx) const {
        ...
        // centers.
        virtual float centerCenterDist2(unsigned short c1, unsigned short c2) const {
            #ifdef COUNT_DISTANCES
            ++numDistances;
            #endif
