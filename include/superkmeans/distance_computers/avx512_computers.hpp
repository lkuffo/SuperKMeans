...
template <DistanceFunction alpha, Quantization q> class SIMDComputer {};

template <> class SIMDComputer<l2, u8> {
  public:
    using distance_t = skmeans_distance_t<u8>;
    using data_t     = DataType_t<u8>;
            ...
            if constexpr (!SKIP_PRUNED) {
                ...
                for (; i + 16 <= n_vectors; i += 16) {
                    ...
                    vec2_u8 = _mm512_loadu_si512(
                        &data[offset_to_dimension_start +
                              i * 4]); // This 4 is because everytime I read 4 dimensions
                    diff_u8 = _mm512_or_si512(_mm512_subs_epu8(vec1_u8, vec2_u8),
                                              _mm512_subs_epu8(vec2_u8, vec1_u8));
                    _mm512_store_epi32(&distances_p[i],
                                       _mm512_dpbusds_epi32(res, diff_u8, diff_u8));
                }
                ...
                for (; i + 8 <= n_vectors; i += 8) {
                    ...
                    y_vec2_u8 = _mm256_loadu_epi8(
                        &data[offset_to_dimension_start +
                              i * 4]); // This 4 is because everytime I read 4 dimensions
                    y_diff_u8 = _mm256_or_si256(_mm256_subs_epu8(y_vec1_u8, y_vec2_u8),
                                                _mm256_subs_epu8(y_vec2_u8, y_vec1_u8));
                    _mm256_store_epi32(&distances_p[i],
    ...
    static void Vertical(const data_t* SKM_RESTRICT query, const data_t* SKM_RESTRICT data,
        ...
        for (size_t dim_idx = start_dimension; dim_idx < end_dimension; dim_idx += 4) {
            ...
            for (int i = 0; i < 4; ++i) { // total: 64 vectors (4 iterations of 16 vectors) * 4
                ...
                const __m512i diff_u8 = _mm512_or_si512(_mm512_subs_epu8(vec1_u8, vec2_u8),
                                                        _mm512_subs_epu8(vec2_u8, vec1_u8));
    ...
    simsimd_l2sq_u8_ice_cycle:
        ...
        __m512i d_u8_vec = _mm512_or_si512(_mm512_subs_epu8(a_u8_vec, b_u8_vec),
                                           _mm512_subs_epu8(b_u8_vec, a_u8_vec));
...
template <> class SIMDComputer<l2, f32> {
  public:
    ...
    static void GatherDistances(size_t n_vectors, distance_t* distances_p,
    ...
    static void GatherBasedKernel(const data_t* SKM_RESTRICT query, const data_t* SKM_RESTRICT data,
                                  size_t n_vectors, size_t total_vectors, size_t start_dimension,
                                  size_t end_dimension, distance_t* distances_p,
                                  const uint32_t* pruning_positions = nullptr) {
        ...
        for (size_t dimension_idx = start_dimension; dimension_idx < end_dimension;
            ...
            for (; i + 16 < n_vectors; i += 16) {
                ...
                data_vec = _mm512_i32gather_ps(_mm512_load_epi32(&pruning_positions[i]), tmp_data,
                                               sizeof(distance_t));
