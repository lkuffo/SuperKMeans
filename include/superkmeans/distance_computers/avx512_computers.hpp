...
class SIMDComputer<l2, u8> {
   public:
    ...
    static void VerticalPruning(const data_t *SKM_RESTRICT query, const data_t *SKM_RESTRICT data, size_t n_vectors,
                                size_t total_vectors, size_t start_dimension, size_t end_dimension,
                                distance_t *distances_p, const uint32_t *pruning_positions = nullptr) {
        __m512i res;
        __m512i vec2_u8;
        __m512i vec1_u8;
        __m512i diff_u8;
    ...
    static void Vertical(const data_t *SKM_RESTRICT query, const data_t *SKM_RESTRICT data, size_t start_dimension,
    ...
    static distance_t Horizontal(const data_t *SKM_RESTRICT vector1, const data_t *SKM_RESTRICT vector2,
...
class SIMDComputer<l2, f32> {
   public:
    ...
    static void GatherDistances(size_t n_vectors, distance_t *distances_p, const uint32_t *pruning_positions) {
    ...
    static void GatherBasedKernel(const data_t *SKM_RESTRICT query, const data_t *SKM_RESTRICT data, size_t n_vectors,
                                  size_t total_vectors, size_t start_dimension, size_t end_dimension,
                                  distance_t *distances_p, const uint32_t *pruning_positions = nullptr) {
        ...
        for (size_t dimension_idx = start_dimension; dimension_idx < end_dimension; ++dimension_idx) {
            ...
            for (; i + 16 < n_vectors; i += 16) {
                ...
                data_vec = _mm512_i32gather_ps(_mm512_load_epi32(&pruning_positions[i]), tmp_data, sizeof(distance_t));
            ...
            for (; i + 8 < n_vectors; i += 8) {
                ...
                data_vec_m256 =
                    _mm256_i32gather_ps(tmp_data, _mm256_load_epi32(&pruning_positions[i]), sizeof(distance_t));
    ...
    simsimd_l2sq_f32_skylake_cycle:
        ...
        if (num_dimensions) goto simsimd_l2sq_f32_skylake_cycle;
