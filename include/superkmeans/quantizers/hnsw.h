...
/**
 * @brief HNSW-based "quantizer" hack.
 *
 * Not a real quantizer — pipes float vectors through unchanged and
 * resolves FindNearestNeighbor by building a fresh FAISS IndexHNSWFlat
 * over the centroids each call, then querying it for top-1 per data point.
 *
 * Centroid updates and norm computation reuse the f32 path.
 * Pruning is not supported.
 */
class HNSWQuantizer : public IQuantizer<Quantization::f32> {
  public:
    using quantized_t = IQuantizer::quantized_t; // float
    using MatrixR = Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>;
    using VectorR = Eigen::VectorXf;

    HNSWQuantizer(int hnsw_M = 32, int ef_construction = 40, int ef_search = 16,
    ...
    /**
     * @brief Build a fresh HNSW over the centroids and query top-1 per data point.
     *
     * Index is reconstructed on every call (centroids change every iteration).
     */
    void FindNearestNeighbor(
        ...
        SKM_PROFILE_SCOPE("HNSW::FindNearestNeighbor");
        assert(fitted);

        faiss::IndexHNSWFlat index(static_cast<int>(d), hnsw_M, faiss::METRIC_L2);
        index.hnsw.efConstruction = ef_construction;
        index.hnsw.efSearch = ef_search;
        {
            SKM_PROFILE_SCOPE("HNSW::FindNearestNeighbor/construction");
        ...
        if (use_warm_start && first_call_done) {
            // Warm-start path: out_knn currently holds the previous iteration's
            // assignments. Use them as per-query level-0 entry points and skip
            // the hierarchical descent entirely.
            std::vector<faiss::HNSW::storage_idx_t> entry_points(n_x);
            std::vector<float> entry_dists(n_x);
            {
                SKM_PROFILE_SCOPE("HNSW::FindNearestNeighbor/entry_points");
#pragma omp parallel for if (g_n_threads > 1) num_threads(g_n_threads)
            ...
            {
                SKM_PROFILE_SCOPE("HNSW::FindNearestNeighbor/search_level_0");
            ...
            SKM_PROFILE_SCOPE("HNSW::FindNearestNeighbor/search");
            index.search(
                static_cast<faiss::idx_t>(n_x),
                x,
                1,
    ...
    void UpdateCentroids(
        ...
        SKM_PROFILE_SCOPE("HNSW::UpdateCentroids");
        assert(fitted);
